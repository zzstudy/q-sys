...
#if OS_CPU_HOOKS_EN > 0  //karlno modify 
...
#else 
	ptcb->OSTCBTaskName=(void *)0;//modify by karlno
#endif
    OS_EXIT_CRITICAL();
    if (OSRunning == OS_TRUE) {
        OS_Sched();                                     /* Find new highest priority task              */
...
#if 1//OS_TASK_NAME_SIZE > 1 //modify by karlno
INT8U  OSTaskNameGet (INT8U prio, INT8U *pname, INT8U *perr)
{
    ...
    len   = OS_StrCopy(pname, (void *)ptcb->OSTCBTaskName);      /* Yes, copy name from TCB                    */
    OS_EXIT_CRITICAL();
    *perr = OS_ERR_NONE;
    return (len);
}
...
#else//modify by karlno
void  OSTaskNameSet (INT8U prio,const INT8U *pname, INT8U *perr)
#endif
{
#if OS_TASK_NAME_SIZE > 1 //modify by karlno
    INT8U      len;
#endif
    OS_TCB    *ptcb;
#if OS_CRITICAL_METHOD == 3                          /* Allocate storage for CPU status register       */
