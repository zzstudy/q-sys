...
{
	gpCtrlObjNum=&(GetCurrPage()->CtrlObjNum);//µÃµ½¿Ø¼þ¸öÊý½á¹¹Ìå

	gpCurImgTchCon=GetCurrPage()->pImgButtonCon;//ÇÐ»»»ù±¾´¥ÃþÇøÓò¼¯Ö¸Õë
...
static bool ImgTchDisplay(u8 Index,INPUT_EVT_TYPE InType,bool IsDyn)
{
	...
	if(pTouchRegion->ImgPathPrefix[0])//Ö¸¶¨ÁËÂ·¾¶Ç°×º£¬ËµÃ÷ÐèÒªÏÔÊ¾°´¼üÍ¼±ê
	{
		...
		else //²»Ê¹ÓÃ×Ô¶¨ÒåÂ·¾¶£¬Ôòµþ¼ÓÖ÷ÌâÂ·¾¶¼°Ò³ÃæÃûÎÄ¼þ¼Ð
		{
			...
			if(RepSuffix) sprintf((void *)PathBuf,"%s%s/Btn/%s%c%c",Q_GetNowThemePath(),GetCurrPage()->Name,pTouchRegion->ImgPathPrefix,RepSuffix,ImgSuffix);//µÃµ½Â·¾¶
			else sprintf((void *)PathBuf,"%s%s/Btn/%s%c",Q_GetNowThemePath(),GetCurrPage()->Name,pTouchRegion->ImgPathPrefix,ImgSuffix);//µÃµ½Â·¾¶
		}
