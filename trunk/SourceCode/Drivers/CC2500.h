...
//CS
#define CC2500_Select()  {GPIO_ResetBits(CC_CS_IO_GROUP, CC_CS_IO_PIN);while(MISO_IN());}//也许需要等待 SPI_MISO_IN()  
...
typedef enum{
	CRA_ALL=0,//接收所有地址的包
	CRA_FIXED,//接收指定地址的包
	CRA_FIXED_00,//接收指定地址和00广播包
	CRA_FIXED_00_FF,//接收指定地址和00广播包和ff广播包
}CC2500_RECV_ADDR;//默认接收数据的第一个字节表示地址
