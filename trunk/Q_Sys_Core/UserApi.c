...
extern u8 gHeapLayer;//¼ÇÂ¼ÓÃ»§Ò³Ãæ¶ÑÕ»²ãÊıµÄ±äÁ¿
...
void Q_EnableInput(void)
{
	...
	OS_TaskResume(KEYS_TASK_PRIORITY);//»Ö¸´°´¼ü¼à¿ØÏß³Ì
}
...
void Q_DisableInput(void)
{
	...
	OS_TaskSuspend(KEYS_TASK_PRIORITY);//¹ÒÆğ°´¼ü¼à¿ØÏß³Ì
}
...
SYS_MSG Q_GotoPage(PAGE_ACTION PageAction, u8 *Name, int IntParam, void *pSysParam)
{
	...
	if(SysMsg!=SM_State_OK)//Ã»ÕÒµ½¶ÔÓ¦Ò³Ãæ
	{
		if(PageAction==SubPageReturn) PageIdx=GetPageIdxByLayerOffset(1);//Èç¹ûÊÇ×ÓÒ³Ãæ·µ»Ø£¬ÔòÕÒµ½ÉÏÒ»¼¶Ò³Ãæ
		else return SysMsg;
	}
	
	//POPÒ³ÃæÖ»ÔÊĞíÒÔ½øÈë×ÓÒ³ÃæµÄĞÎÊ½½øÈë
	...
	if(SysMsg&SM_NoGoto)//Ò³ÃæµÄSys_Goto_Page´«µİ»ØµÄĞÅÏ¢
